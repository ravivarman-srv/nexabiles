...
Nexabilis talks to the official **WhatsApp® Business Cloud API** (Meta). Each
...
| Permanent access token| `access_token` (System User token — see below)    |

Meta provides a temporary token on the setup page, but it expires after 24 hours. You must generate a **System User (Permanent)** access token instead:

1. **Business Settings → Users → System users → Add**.
2. Name it `nexabilis-system-user`, role **Admin**.
