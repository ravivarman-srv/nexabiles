...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  return (
    <div className="bg-background -m-4 flex h-[calc(100vh-3.5rem)] flex-col overflow-hidden sm:-m-6">
      {/* Top bar. At sub-sm widths the "Active" label is hidden and the
          switch moves to the right of the save button, so the name input
          gets maximum width. */}
      <header className="border-border bg-card flex flex-shrink-0 items-center gap-2 border-b px-3 py-3 sm:gap-3 sm:px-4">
        <button
          type="button"
      ...
      {/* Canvas */}
      <div className="relative flex-1 overflow-x-hidden overflow-y-auto bg-[radial-gradient(circle,#1e293b_1px,transparent_1px)] [background-size:20px_20px]">
...
function AddButton({ onPick }: { onPick: (t: AutomationStepType) => void }) {
  return (
    <div className="relative flex flex-col items-center">
      <div className="bg-muted h-4 w-[2px]" aria-hidden />
      <DropdownMenu>
        <DropdownMenuTrigger
        ...
        <DropdownMenuContent
          align="start"
          className="border-border bg-card max-h-80 min-w-56 overflow-y-auto"
        >
          {ADDABLE_STEPS.map((t) => {
